...
def read_file_fs(file_path, n_machines=2):
    ...
    with open(file_path, 'r') as file:
        ...
        # Divide them into instances with num_jobs_per_instance each
        num_total_jobs = len(parsed_instances)
        n_ops=num_jobs_per_instance*n_machines 
        num_instances = num_total_jobs // n_ops

        for i in range(num_instances):
            
            jobs = []
            start_index = i * n_ops
            end_index = start_index + n_ops
    ...
    print(f"Processed {len(instances)} instances and saved to 'instances_fs_{n_machines}.json'.")
